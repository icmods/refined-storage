...
/* Stable callback reference: ensureInterval compares identity, so a fresh
 * closure per LevelLoaded would log "replaced (callback changed)" and reset
 * the cadence. */
function rsEnsureReconnectInterval() {
	TickScheduler.global.ensureInterval("rs:reconnect", 20, rsReconnectPass);
}
...
/* Extra mode: tasks are NOT persisted; before the world save every in-flight buffer
 * goes back to the network (items reach the disks; their payloads flush separately).
 * Savecore mode (flag off) keeps the task persistence below. */
function rsFlushAllTaskBuffers() {
	if (!(typeof RS_DISK_DATA_IN_EXTRA != 'undefined' && RS_DISK_DATA_IN_EXTRA)) return;
	for (var netId in RSNetworks) {
		var net = RSNetworks[netId];
		var info = net && net.info;
		if (!info || !info.craftingTasks) continue;
		var tasks = info.craftingTasks.slice();
		for (var ti = 0; ti < tasks.length; ti++) {
			var task = tasks[ti];
			if (!task || typeof task.flushBuffer != 'function') continue;
			task.cancelled = true;
			try { task.flushBuffer(); }
			catch (e) { try { Logger.Log('task buffer flush failed on level leave: ' + e, 'RefinedStorageError'); } catch (e2) {} }
		}
	}
}
Callback.addCallback("LevelPreLeft", rsFlushAllTaskBuffers);

/* Task persistence — active only when RS_DISK_DATA_IN_EXTRA is off (save on). */
	...
	SaveCore.scope("RSCraftingTasks", {
		...
		serialize: function () {
			...
			for (var i in RSNetworks) {
				...
				var controllerCoords = info.controllerCoords || searchController_net(i);
...
Callback.addCallback("LevelLeft", function () {
	...
	if (typeof StorageCore !== 'undefined' && StorageCore && typeof StorageCore.resetIndex === 'function') StorageCore.resetIndex();
	if (typeof NetworkTimer !== 'undefined') NetworkTimer.reset();
	if (typeof rsDisposeUiRuntime === 'function') rsDisposeUiRuntime();
	...
	if (typeof pistonsMoveTicks__ !== 'undefined') pistonsMoveTicks__ = {};
});
