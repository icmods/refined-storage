LIBRARY({
    ...
    shared: true,
    api: "CoreEngine",
    dependencies: ["CoreKit:2", "EventBus:2", "TickScheduler:2"]
});
 ...
 * options.includeSelf (default true): when false the starting coords are NEVER
 ...
 */
_TopoGraph.prototype.searchController = function (coords, dim, out, options) {
    ...
    var includeSelf = !(options && options.includeSelf === false);
 ...
 */
_TopoGraph.prototype.floodAssign = function (controllerCoords, dim, out, options) {
    dim = dim === undefined || dim === null ? 0 : dim;
    if (!controllerCoords || !controllerCoords.blockSource) return null;
    var cause = (options && options.cause) || "assign";
    var destructive = !(options && options.destructive === false);
    if (this._opDepth > 0) {
        this._deferOp({
            type: "flood",
...
_TopoGraph.prototype._floodAssign = function (controllerCoords, dim, out, options) {
    dim = dim === undefined || dim === null ? 0 : dim;
...
_TopoGraph.prototype.refloodComponentOf = function (coords, dim, blockSource, options) {
    dim = dim === undefined || dim === null ? 0 : dim;
    var cKey = this.nodeIndex[_topoKey(dim, coords)];
    ...
    if (this._opDepth > 0) {
        this._deferOp({
            ...
            dim: dim,
            cause: (options && options.cause) || "reflood",
    ...
    this.floodAssign({ x: component.coords.x, y: component.coords.y, z: component.coords.z, blockSource: blockSource }, dim, null,
        { cause: (options && options.cause) || "reflood", destructive: !!(options && options.destructive) });
    return true;
};
