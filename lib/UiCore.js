...
/* Guarded logger: safe when Logger is absent (Node tests). */
function _uiWarn(msg) {
    try {
        if (typeof Logger != "undefined" && Logger && Logger.Log) Logger.Log(String(msg), "UiCore");
    } catch (e) {}
}

/*
 * Optional host hooks: options.isActive() / options.isAlive(element) — checked before
 * any host call (switchPage/setPosition/onClose). Absent hooks = current
 * behavior. A throwing hook is treated as "not alive" (fail-closed).
 */
function _uiHookAllowed(options, element) {
    if (typeof options.isActive == "function") {
        try { if (!options.isActive()) return false; }
        catch (e) { _uiWarn("isActive hook failed: " + e); return false; }
    }
    if (typeof options.isAlive == "function") {
        try { if (!options.isAlive(element)) return false; }
        catch (e) { _uiWarn("isAlive hook failed: " + e); return false; }
    }
    return true;
}
 ...
 */
UiCore.attachSwipe = function (options) {
    var anchor = options.variant === "anchor";
    var dragEnabled = options.drag !== false;
    var _th = options.thresholds || {};
    ...
    return function (element, event) {
        if (!_uiHookAllowed(options, element)) return;
        var s = options.state;
        if (anchor) {
            if (event.type == "DOWN") {
                s.swipeY = event.y;
                s.swipeSum = 0;
                s.swipeDir = 0;
            } else if (s.swipeY !== false && s.swipeY != null && event.type == "MOVE") {
                ...
                s.swipeY = event.y;
                if (moved !== 0) {
                    var anchorPage = options.getLastPage() + moved;
                    if (options.snapAfterSwipe === true) {
                        if (options.switchPage(anchorPage, false, false)) {
                            var anchorPages = options.pagination.getPages(options.itemCount());
                            var anchorEl = options.getElement(options.sliderElementName);
                            if (anchorEl && anchorEl.setPosition) {
                                anchorEl.setPosition(options.sliderX, options.pagination.getCoordsFromPage(anchorPage, anchorPages));
                            }
                        }
                    } else {
                        options.switchPage(anchorPage, false, false);
                    }
                }
            } else if (s.swipeY !== false && s.swipeY != null && _release(event.type)) {
                s.swipeY = false;
                s.swipeSum = 0;
            }
        } else {
            if (event.type == "DOWN" && !s.moving &&
                event.x > options.bounds.xStart && event.x < options.bounds.xEnd &&
                event.y > options.bounds.yStart && event.y < options.bounds.yEnd) {
                s.swipeY = event.y;
                ...
                function moveSwitchPage_(_n) {
                    var layoutPage = options.getLastPage() + (_n ? 1 : -1);
                    if (options.snapAfterSwipe === true) {
                        if (options.switchPage(layoutPage, false, false)) {
                            var layoutPages = options.pagination.getPages(options.itemCount());
                            var layoutEl = options.getElement(options.sliderElementName);
                            if (layoutEl && layoutEl.setPosition) {
                                layoutEl.setPosition(options.sliderX, options.pagination.getCoordsFromPage(layoutPage, layoutPages));
                            }
                        }
                    } else {
                        options.switchPage(layoutPage, false, false);
                    }
                }
        ...
        if (!s.moving) return;
        var sliderY = event.y - options.sliderScale * 15 / 2;
        if (!_release(event.type)) {
            var pages = options.pagination.getPages(options.itemCount());
            var page = options.pagination.getPageFromCoords({ y: sliderY }, pages);
            var el = options.getElement(options.sliderElementName);
            if (el && el.setPosition) {
                el.setPosition(options.sliderX, Math.max(Math.min(sliderY, options.maxY), options.sliderStartY));
            }
            options.switchPage(page, false, true);
        }
        if (_release(event.type)) {
            s.moving = false;
            var pages2 = options.pagination.getPages(options.itemCount());
            var page2 = options.pagination.getPageFromCoords({ y: sliderY }, pages2);
            options.switchPage(page2, false, false);
            if (options.snapSliderOnRelease !== false) {
                var el2 = options.getElement(options.sliderElementName);
                if (el2 && el2.setPosition) {
                    el2.setPosition(options.sliderX, options.pagination.getCoordsFromPage(page2, pages2));
                }
            }
        }
    };
 ...
 * }
 */
UiCore.attachSlider = function (options) {
    return function (element, event) {
        if (!_uiHookAllowed(options, element)) return;
        if (event.type == "DOWN") {
            options.state.moving = true;
        }
        // CLICK arrives without UP on some engine paths: release the slider too.
        if (event.type == "UP" || event.type == "CANCEL" || event.type == "CLICK") {
            options.state.moving = false;
        }
        if (event.type == "CLICK") {
            var pages = options.pagination.getPages(options.itemCount());
            var page = options.pagination.getPageFromCoords(event, pages);
            if (options.switchOnClick !== false) {
                options.switchPage(page, false, false);
            }
            if (options.snapOnClick === true) {
                var el = options.getElement(options.sliderElementName);
                if (el && el.setPosition) {
                    el.setPosition(options.sliderX, options.pagination.getCoordsFromPage(page, pages));
                }
            }
        }
    };
 ...
 * options: { onSearch(keywordOrFalse, handler, container), title, textLabel,
 ...
 */
UiCore.searchBox = function (x, y, width, options) {
    options = options || {};
    if (typeof options.onSearch != "function") _uiWarn("searchBox: options.onSearch is not a function; search disabled");
    var dialog = null;
    var ctx = { onSearch: typeof options.onSearch == "function" ? options.onSearch : null };
    var frame = {
        type: "frame",
        x: x,
        y: y,
        width: width,
        height: options.height !== undefined ? options.height : 30,
        bitmap: options.bitmap || "search_bar",
        scale: 0.8,
        clicker: {
                        ...
                        try {
                            ...
                            var applyKeyword = function (keyword) {
                                ...
                                try {
                                    ...
                                    _uiWarn("searchBox onSearch failed: " + err);
                                }
                            };
                            var builder = new android_.app.AlertDialog.Builder(UI_.getContext())
                                .setTitle(options.title || "Please type the keywords")
                                .setView(editText)
                                .setPositiveButton(options.positiveLabel || "Search", {
                                    onClick: function () {
                            ...
                            _uiWarn("searchBox dialog failed: " + e);
                        }
                    }
                }));
            }
    ...
    var text = {
        ...
        y: y + 1,
        z: 100,
        text: options.textLabel || "Search",
        font: {
            color: options.fontColor !== undefined ? options.fontColor : (typeof android != "undefined" && android.graphics && android.graphics.Color ? android.graphics.Color.WHITE : 0),
            shadow: 0.5,
            size: options.fontSize !== undefined ? options.fontSize : 20
        }
    };
    return {
        frame: frame,
        text: text,
        /** Dismiss the native dialog if it is still open (no-op otherwise). */
        close: function () {
            if (!dialog) return;
            try { dialog.dismiss(); } catch (e) { _uiWarn("searchBox close failed: " + e); }
            dialog = null;
        }
    };
};
 ...
 * APIs are unavailable. options: { title, positiveLabel, negativeLabel, onResult(ok) }.
 */
UiCore.confirm = function (message, options) {
    options = options || {};
    if (!_uiDialogAvailable()) return false;
    if (typeof options.isOpen == "function") {
        try { if (!options.isOpen()) return false; }
        catch (e) { _uiWarn("confirm isOpen failed: " + e); return false; }
    }
    try {
            ...
            run: function () {
                try {
                    var builder = new android.app.AlertDialog.Builder(ctx)
                        ...
                        .setPositiveButton(options.positiveLabel || "OK", {
                            onClick: function () { if (typeof options.onResult == "function") options.onResult(true); }
                        })
                        .setNegativeButton(options.negativeLabel || "Cancel", {
                            onClick: function () { if (typeof options.onResult == "function") options.onResult(false); }
                        });
                    if (options.title) builder.setTitle(String(options.title));
                    builder.show();
                } catch (e) {
                    _uiWarn("confirm dialog failed: " + e);
                }
            }
        }));
        return true;
 ...
 * the engine APIs are unavailable. options: { title, positiveLabel, onClose }.
 */
UiCore.alert = function (message, options) {
    options = options || {};
    if (!_uiDialogAvailable()) return false;
    if (typeof options.isOpen == "function") {
        try { if (!options.isOpen()) return false; }
        catch (e) { _uiWarn("alert isOpen failed: " + e); return false; }
    }
    try {
        var ctx = UI.getContext();
        ctx.runOnUiThread(new java.lang.Runnable({
            run: function () {
                try {
                    var builder = new android.app.AlertDialog.Builder(ctx)
                        .setMessage(String(message == undefined ? "" : message))
                        .setPositiveButton(options.positiveLabel || "OK", {
                            onClick: function () { if (typeof options.onClose == "function") options.onClose(); }
                        });
                    if (options.title) builder.setTitle(String(options.title));
                    builder.show();
                } catch (e) {
                    _uiWarn("alert dialog failed: " + e);
                }
            }
        }));
        return true;
 ...
 * options: { base: { type, size, ... }, maxCount }
 */
UiCore.createSlotPool = function (options) {
    options = options || {};
    var pool = {};
    var count = 0;
    var maxCount = options.maxCount > 0 ? options.maxCount : 0;
    function own(name) { return Object.prototype.hasOwnProperty.call(pool, name); }
    ...
    return {
        ...
        get: function (name) {
            ...
            if (!own(name)) {
                if (maxCount > 0 && count >= maxCount) return null;
                var desc = _uiClone(options.base) || {};
                desc.x = -1000;
                desc.y = -1000;
                pool[name] = desc;
                count++;
        ...
        has: function (name) { return allowed(name) && own(name); },
        count: function () { return count; },
        maxCount: function () { return maxCount; },
        /** Move an active descriptor to its screen position. */
        setPosition: function (name, x, y) {
            var desc = own(name) ? pool[name] : null;
        ...
        parkAll: function () {
            ...
            for (var i = 0; i < names.length; i++) this.park(names[i]);
        },
        pool: pool
    };
};
 ...
 */
UiCore.applyDarken = function (elements, names, predicate, options) {
    ...
    if (changed > 0 && options && options.window && typeof options.window.forceRefresh == "function") {
        options.window.forceRefresh();
    }
    return changed;
};
...
function _UiDeltaBinder(container, options) {
    this.container = container;
    this.options = options || {};
    this._cache = { scale: {}, text: {}, binding: {} };
    this._changed = false;
    this._failed = {};
}
 ...
 * changed. Cache invalidated when the GUI opens; per-kind caches (scale/text/
 * binding) so the same element name can carry independent values. A throwing
 * container call is logged once per id and reported to `options.onError`, stays
 * uncached and is retried on the next tick.
 */
UiCore.DeltaBinder = {
    wrap: function (container, options) {
        return new _UiDeltaBinder(container, options);
    }
};

_UiDeltaBinder.prototype._noteFailure = function (e, id) {
    if (!this._failed[id]) {
        this._failed[id] = true;
        _uiWarn("DeltaBinder " + id + " failed: " + e);
    }
    if (typeof this.options.onError == "function") {
        try { this.options.onError(e, id); } catch (e2) {}
    }
};

_UiDeltaBinder.prototype._set = function (kind, name, value) {
    if (!this.canSend()) return;
    var cache = this._cache[kind];
    ...
    try {
        ...
        else this.container.setText(name, value);
    } catch (e) {
        this._noteFailure(e, kind + ":" + name);
        return;
    }
    cache[name] = value;
    this._changed = true;
...
_UiDeltaBinder.prototype.setBinding = function (name, key, value) {
    if (!this.canSend()) return;
    var cache = this._cache.binding;
    var cacheKey = name + "#" + key;
    if (cache[cacheKey] === value) return;
    try { this.container.setBinding(name, key, value); }
    catch (e) { this._noteFailure(e, "binding:" + cacheKey); return; }
    cache[cacheKey] = value;
    this._changed = true;
};

/**
 * True when this container may be mutated/sent to from the current side.
 * Client containers (`isServer === false`) must not sendChanges; unknown
 * containers keep the previous behavior (true).
 */
_UiDeltaBinder.prototype.canSend = function () {
    var isServer = this.container && this.container.isServer;
    if (typeof isServer == "boolean") return isServer;
    return true;
};
...
function _uiContentSignature(content) {
    ...
    for (var n in content.elements) {
        ...
        parts.push("e", e.type, e.x, e.y, e.size, e.width, e.height, e.scale, e.bitmap, e.bitmap2,
            _uiSignatureValue(e.source), e.color, e.z, e.text, (e.font && e.font.size) || 0);
    }
    for (var d in content.drawing) {
        var o = content.drawing[d];
        parts.push("d", o.type, o.x, o.y, o.size, o.width, o.height, o.scale, o.bitmap, o.bitmap2, o.color, o.z);
    }
    return parts.join("\u0001");
}

/** Shallow copy of a measure result: the cached object is never handed out. */
function _uiCopyBox(b) {
    return { scrollX: b.scrollX, scrollY: b.scrollY, minX: b.minX, minY: b.minY, width: b.width, height: b.height };
}
...
UiCore.measureContent = function (content) {
    ...
    if (cached !== undefined) return _uiCopyBox(cached);
    ...
    return _uiCopyBox(result);
};

/** Scale element/drawing descriptors in place (text font, frames, sizes, x/y). scale must be > 0. */
UiCore.scaleContent = function (content, scale) {
    if (!content || typeof scale != "number" || !(scale > 0) || scale == 1) return content;
 ...
 * options.center {x,y}. Returns { scale, size }.
 */
UiCore.fitContent = function (content, options) {
    options = options || {};
    var padding = options.padding || 0;
    var maxWidth = options.maxWidth != null ? options.maxWidth - padding * 2 : 0;
    var maxHeight = options.maxHeight != null ? options.maxHeight - padding * 2 : 0;
    var size = UiCore.measureContent(content);
    var scale = options.maxScale != null ? options.maxScale : 1;
    ...
    if (options.minScale != null && scale < options.minScale) scale = options.minScale;
    if (scale !== 1) {
        UiCore.scaleContent(content, scale);
        size = UiCore.measureContent(content);
    }
    if (options.center && typeof options.center == "object") {
        UiCore.offsetContent(content, options.center.x - (size.minX + size.width / 2), options.center.y - (size.minY + size.height / 2));
        size = UiCore.measureContent(content);
    }
 ...
 * options: { tabs: [{ id, bitmap?, iconBitmap?, icon?, onClick? }], x, y, size,
...
UiCore.tabs = function (options) {
    ...
    var maxVisible = options.maxVisible != null ? options.maxVisible : tabs.length;
    ...
    for (var i = offset; i < end; i++) {
        ...
        var frameBitmap = tab.bitmap || options.bitmap;
        if (frameBitmap) {
            elements[prefix + id] = { type: "frame", x: x, y: ty, width: size, height: size, bitmap: frameBitmap };
        }
        var iconBitmap = tab.iconBitmap || options.iconBitmap;
 ...
 * DOWN/CLICK inside `bounds` returns false; outside calls onClose() ONCE and
 * returns true (consumed). options: { isOpen(): boolean, isActive(), isAlive(el) } —
 * when `isOpen()` returns false the close is skipped. Bounds are in window
 * coordinates.
 */
UiCore.outsideClose = function (bounds, onClose, options) {
    bounds = bounds || {};
    options = options || {};
    var fired = false;
    return function (element, event) {
        if (event.type != "DOWN" && event.type != "CLICK") return false;
        if (!_uiHookAllowed(options, element)) return false;
        var x = event.x, y = event.y;
        var inside = x >= bounds.xStart && x <= bounds.xEnd && y >= bounds.yStart && y <= bounds.yEnd;
        if (inside) return false;
        if (!fired && typeof onClose == "function") {
            if (typeof options.isOpen == "function") {
                try {
                    if (!options.isOpen()) return true;
                } catch (e) {
                    _uiWarn("outsideClose isOpen failed: " + e);
                    return true;
                }
            }
            fired = true;
            try {
                ...
                _uiWarn("outsideClose onClose failed: " + e);
                throw e;
            }
        }
        return true;
    ...
    try {
        if (typeof UI != "undefined" && UI.getContext) {
            ...
            if (ctx) {
                ...
                if (result.width > 0 && result.height > 0) _uiDisplayCache = result;
                return result;
            }
        }
    } catch (e) {
        _uiWarn("display metrics failed: " + e);
    }
    try {
        ...
        _uiWarn("display fallback failed: " + e);
    }
    if (result.width > 0 && result.height > 0) _uiDisplayCache = result;
    return result;
};
...
function _uiApplyDimension(unit, value) {
    try {
        ...
        _uiWarn("dimension conversion failed: " + e);
    }
...
var _uiLiveRefs = [];
var _uiLiveCache = {};
var _uiLiveRefCap = 64;

/** Container identity for the live-element cache (never tags the object). */
function _uiLiveRefId(container) {
    for (var i = 0; i < _uiLiveRefs.length; i++) {
        if (_uiLiveRefs[i] === container) return i;
    }
    if (_uiLiveRefs.length >= _uiLiveRefCap) {
        _uiLiveRefs.length = 0;
        _uiLiveCache = {};
    }
    _uiLiveRefs.push(container);
    return _uiLiveRefs.length - 1;
}

/** One Minecraft tick (50 ms) by default; options.tick overrides (number/provider). */
function _uiLiveTick(options) {
    var t = (options && options.tick !== undefined) ? options.tick : null;
    if (typeof t == "function") {
        try { t = t(); } catch (e) { t = null; }
    }
    if (typeof t == "number" && isFinite(t)) return t;
    try { return Math.floor(new Date().getTime() / 50); } catch (e) { return 0; }
}

/** Best-effort release check: only a positive flag marks the element dead. */
function _uiElementReleased(element) {
    try {
        if (typeof element.isReleased == "function") return !!element.isReleased();
        if (element.isReleased === true || element.released === true) return true;
    } catch (e) {}
    return false;
}

function _uiResolveLiveElement(container, name) {
    if (!container || !name) return null;
    var el = null;
    try {
        if (typeof container.getElements == "function") {
            var map = container.getElements();
            if (map) el = (typeof map.get == "function") ? map.get(name) : map[name];
        }
    } catch (e) { el = null; }
    if (!el) {
        try {
            var adapter = (typeof container.getUiAdapter == "function") ? container.getUiAdapter() : null;
            if (adapter && typeof adapter.getElement == "function") el = adapter.getElement(name);
        } catch (e) { el = null; }
    }
    if (!el && typeof container.getElement == "function") {
        try { el = container.getElement(name); } catch (e) { el = null; }
    }
    if (!el || _uiElementReleased(el)) return null;
    return el;
}

/**
 * Live element lookup (anti stale references, model rsGetLiveElement): resolve
 * `container`/`name` at call time; never keep the returned element across a
 * refresh. Resolution order: getElements() map → getUiAdapter().getElement →
 * getElement(name). A released element is treated as missing when the host
 * object exposes isReleased()/released (best effort: current engines filter it
 * inside getElement and do not expose the flag in JS).
 *
 * Cached for ONE tick: options.tick (number or provider) invalidates on change;
 * the default is Math.floor(Date.now() / 50) (one Minecraft tick).
 * options.cache === false bypasses the cache; UiCore.dispose() clears it. The
 * cache holds up to 64 container identities (then it resets) and never tags
 * the container object.
 */
UiCore.liveElement = function (container, name, options) {
    if (!container || !name) return null;
    options = options || {};
    if (options.cache === false) return _uiResolveLiveElement(container, name);
    var tick = _uiLiveTick(options);
    var key = _uiLiveRefId(container) + "|" + name;
    var entry = _uiLiveCache[key];
    if (entry && entry.tick === tick) return entry.el;
    var el = _uiResolveLiveElement(container, name);
    _uiLiveCache[key] = { tick: tick, el: el };
    return el;
};
...
var _uiExclusive = {};
var _uiThreadIdProvider = null;

/**
 * Thread-id provider for UiCore.exclusive (optional). Host example:
 *   UiCore.setThreadIdProvider(function () {
 *       return java.lang.Thread.currentThread().getName();
 *   });
 * Default provider returns "single" (every call is the same thread).
 */
UiCore.setThreadIdProvider = function (fn) {
    _uiThreadIdProvider = (typeof fn == "function") ? fn : null;
};

function _uiThreadId() {
    if (_uiThreadIdProvider) {
        try {
            var id = _uiThreadIdProvider();
            if (id !== null && typeof id != "undefined") return String(id);
        } catch (e) {}
        return "?";
    }
    return "single";
...
function _uiExclusiveGet(key, create) {
    if (typeof key != "string" || !key.length) return null;
    if (key == "__proto__" || key == "constructor" || key == "prototype") return null;
    if (!Object.prototype.hasOwnProperty.call(_uiExclusive, key)) {
        if (create === false) return null;
        _uiExclusive[key] = {
            busy: false, owner: null, depth: 0, runs: 0, deferred: 0,
            pendingList: [], lastOwner: null, lastDeferrer: null
        };
    }
    return _uiExclusive[key];
}

function _uiPendingUpsert(lock, wrapper, self, args, deferrer) {
    /* One pending entry per wrapped function (coalesced: latest arguments win),
     * but DIFFERENT functions (updateGui / switchPage / setItemInfoSlot) keep
     * separate entries so no action type is lost.
     * The list is only mutated in place (never reassigned) so a concurrent
     * queueCall from another thread can never observe a hole/undefined entry. */
    var list = lock.pendingList;
    for (var i = 0; i < list.length; i++) {
        var entry = list[i];
        if (!entry) continue;
        if (entry.wrap === wrapper) {
            entry.self = self;
            entry.args = args;
            entry.deferrer = deferrer;
            return;
        }
    }
    list.push({
        wrap: wrapper, self: self, args: args, deferrer: deferrer,
        tag: (typeof wrapper.__uiTag == "string" && wrapper.__uiTag) ? wrapper.__uiTag : null
    });
}

/**
 * Wrap `fn` with cooperative, non-blocking mutual exclusion per `key`:
 *   - same thread as the current owner -> re-entrant, runs normally;
 *   - another thread while busy -> QUEUED: `fn` is NOT called, its arguments
 *     are remembered (one pending entry per wrapped function, latest args
 *     win) and the call returns false; replayPending(key) runs the queued
 *     calls later, typically from the next tick.
 * Heavy work stays on its own thread (no UI-thread marshaling, see safeRun).
 * Optional `tag` names the logical function across re-wraps (e.g. "updateGui"):
 * on re-wrap only pending calls carrying the same tag are dropped.
 * For a strict single-executor policy, the host queues all calls that do NOT
 * come from the executor thread via queueCall(key, wrapper, this, args).
 */
UiCore.exclusive = function (key, fn, tag) {
    if (typeof fn != "function") return fn;
    var lock = _uiExclusiveGet(key);
    if (!lock) return fn;
    var wrapper = function () {
        var tid = _uiThreadId();
        if (lock.busy) {
            // Unknown thread identity ("?") is never trusted as the owner: a
            // provider failure must fail closed, not run cross-thread code.
            if (lock.owner === tid && tid !== "?") {
                lock.depth++;
                try { return fn.apply(this, arguments); }
                finally { lock.depth--; }
            }
            lock.deferred++;
            lock.lastOwner = lock.owner;
            lock.lastDeferrer = tid;
            _uiPendingUpsert(lock, wrapper, this, arguments, tid);
            return false;
        }
        lock.busy = true;
        lock.owner = tid;
        ...
        lock.runs++;
        try { return fn.apply(this, arguments); }
        finally { lock.depth = 0; lock.busy = false; lock.owner = null; }
    };
    wrapper.__uiKey = key;
    wrapper.__uiTag = (typeof tag == "string" && tag) ? tag : null;
    /* Re-wrapped function (GUI reopened): with a stable `tag` only the pending
     * calls of the SAME logical function are dropped (queued calls of the other
     * wrappers on this key stay valid); without a tag the legacy behavior is
     * kept (all pending calls belong to the previous window instance). */
    if (wrapper.__uiTag) {
        var w = 0;
        for (var i = 0; i < lock.pendingList.length; i++) {
            var e = lock.pendingList[i];
            if (e && e.tag !== wrapper.__uiTag) lock.pendingList[w++] = e;
        }
        lock.pendingList.length = w;
    } else {
        lock.pendingList.length = 0;
    }
    return wrapper;
};
 ...
 * executor" policy: calls from other threads are queued and replayed on the
 * executor thread). The wrapper must be the one produced by
 * `UiCore.exclusive(key, fn)` (tag check) — foreign wrappers are rejected.
 * Returns true when queued.
 */
UiCore.queueCall = function (key, wrapper, self, args) {
    var lock = _uiExclusiveGet(key);
    if (!lock || typeof wrapper != "function") return false;
    if (typeof wrapper.__uiKey != "string" || wrapper.__uiKey !== key) return false;
    _uiPendingUpsert(lock, wrapper, self, args, _uiThreadId());
    return true;
};

/** True when at least one queued call waits for `key` (read-only; never creates state). */
UiCore.hasPending = function (key) {
    var lock = _uiExclusiveGet(key, false);
    return !!(lock && lock.pendingList.length);
};

/**
 * Run queued calls for `key` on the CURRENT thread (oldest first).
 * options: { expectedOwner: string, onError(error, entry) } — entries deferred by a
 * different thread stay queued; a throwing entry is reported (onError or the
 * guarded log) and dropped, the remaining entries still run.
 * Returns the number of calls executed.
 */
UiCore.replayPending = function (key, options) {
    options = options || {};
    var lock = _uiExclusiveGet(key, false);
    if (!lock || !lock.pendingList.length) return 0;
    // In-place compaction (the list object is never swapped) so a concurrent
    // queueCall from another thread never observes an undefined entry.
    var list = lock.pendingList;
    var ran = 0, guard = 0, w = 0, i = 0;
    for (; i < list.length && guard < 64; i++) {
        guard++;
        var entry = list[i];
        if (!entry) continue;
        if (options.expectedOwner !== undefined && entry.deferrer !== options.expectedOwner) {
            list[w++] = entry;
            continue;
        }
        try {
            entry.wrap.apply(entry.self, entry.args);
            ran++;
        } catch (e) {
            if (typeof options.onError == "function") {
                try { options.onError(e, entry); } catch (e2) {}
            } else {
                _uiWarn("replayPending failed: " + e);
            }
        }
    }
    for (; i < list.length; i++) {
...
/**
 * Forget all queued calls for `key` without replaying them (e.g. the window was
 * closed while calls were pending — replaying against a dead window is a crash
 * hazard). Read-only; returns the number of dropped calls.
 */
UiCore.dropPending = function (key) {
    var lock = _uiExclusiveGet(key, false);
    if (!lock || !lock.pendingList.length) return 0;
    var dropped = lock.pendingList.length;
    lock.pendingList.length = 0;
    return dropped;
};

/** Remove the lock and its counters for `key` (diagnostics); queued calls are dropped. */
UiCore.forget = function (key) {
    if (!_uiExclusiveGet(key, false)) return false;
    delete _uiExclusive[key];
    return true;
};

/** Counters snapshot for `key` (diagnostics/tests; never creates state). */
UiCore.exclusiveStats = function (key) {
    var lock = _uiExclusiveGet(key, false);
    if (!lock) return null;
    return {
        runs: lock.runs, deferred: lock.deferred, pending: lock.pendingList.length,
        owner: lock.owner, depth: lock.depth,
        lastOwner: lock.lastOwner, lastDeferrer: lock.lastDeferrer
    };
};
...
UiCore.safeRun = function (fn, logTag) {
    ...
    run();
    _uiWarn("safeRun fell back to a direct call (no UI context): " + (logTag || "UiCore"));
    return false;
};

/**
 * LevelLeft cleanup: drops every queued exclusive call, clears the memo caches
 * (text width / measure / display / live-element) and resets the thread-id
 * provider (re-set it from the next LevelLoaded). Idempotent; returns the
 * number of dropped calls.
 */
UiCore.dispose = function () {
    var dropped = 0;
    for (var key in _uiExclusive) {
        var lock = _uiExclusive[key];
        if (lock && lock.pendingList && lock.pendingList.length) {
            dropped += lock.pendingList.length;
            lock.pendingList.length = 0;
        }
    }
    _uiTextWidthCache = Object.create(null);
    _uiTextWidthKeys = [];
    _uiMeasureCache = Object.create(null);
    _uiMeasureKeys = [];
    _uiDisplayCache = null;
    _uiLiveCache = {};
    _uiLiveRefs.length = 0;
    _uiThreadIdProvider = null;
    return dropped;
};
