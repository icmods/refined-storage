 ...
 */
CompatCore.whenAvailable = function (apiName, callback, options) {
    options = options || {};
    if (typeof ModAPI == "undefined" || !ModAPI || typeof ModAPI.addAPICallback != "function") return null;
    apiName = String(apiName);
    if (options.once !== false && options.hostId) {
        var hostKey = String(options.hostId);
    ...
    var timeoutTicks = Number(options.timeout);
    if (isFinite(timeoutTicks) && timeoutTicks > 0) {
        timer = _compatArmTimer(timeoutTicks, function () {
            ...
            if (typeof options.onTimeout == "function") {
                try { options.onTimeout(apiName); }
                catch (e) { _compatLog("CompatCore: onTimeout failed for '" + apiName + "': " + e); }
            }
        });
        if (!timer && typeof options.onTimeout == "function") {
