...
function _scoreDefaults(opts) {
    if (typeof opts.defaults == "function") {
        try { return opts.defaults(); } catch (e) { return {}; }
    }
    return {};
}

/**
 * scope(name, opts) → store { data, name, version }
 * opts: {
 ...
 */
SaveCore.scope = function (name, opts) {
    opts = opts || {};
    ...
    Saver.addSavesScope(name, function (scope) {
        ...
        var version;
        var data;
        if (!isEnvelope && typeof opts.legacy == "function") {
            ...
            try { data = opts.legacy(raw); } catch (e) {
        ...
        if (version > store.version) {
            ...
            store.data = _scoreDefaults(opts);
            return;
        }

        if (version < store.version && opts.migrations) {
            while (version < store.version) {
                var migrate = opts.migrations[version + 1];
        ...
        if (version < store.version && !opts.migrations && !usedLegacy && hadVersion) {
        ...
        if (typeof opts.deserialize == "function") {
            try { data = opts.deserialize(data); }
            catch (e) {
        ...
        var valid = true;
        if (opts.validate) {
            try { valid = !!opts.validate(data); } catch (e) { valid = false; }
        }
        if (!valid) {
        ...
        store.data = (data === null || data === undefined) ? _scoreDefaults(opts) : data;
    }, function () {
        // Store JSON-pure data; avoid JS circular references.
        var dataToSave = (store.data === null || store.data === undefined) ? _scoreDefaults(opts) : store.data;
        ...
        if (typeof opts.serialize == "function") {
            try { dataToSave = opts.serialize(dataToSave); }
    ...
    Callback.addCallback("LevelLeft", function () {
        if (typeof opts.onLevelLeft == "function") {
            try { opts.onLevelLeft(store); } catch (e) {}
        }
        store.data = null;
    });
...
SaveCore.resetAll = function () {
    for (var name in _SCORE_scopes) _SCORE_scopes[name].data = null;
};
