...
function _TS_Scheduler(driverName) {
    ...
    this._rrLane = 0;
    this._warnedNonFn = Object.create(null);
    var self = this;
    if (this._driver) {
        Callback.addCallback(this._driver, function () {
            ...
            try {
                ...
                _tsLog("tick driver error isolated: " + err);
            }
        });
    }
}
...
/** Reserve an internal task name (registration with a reserved name requires options.internal). */
...
/** Log-once guard for invalid primitive callbacks (per primitive+key, cleared on resetAll/destroy). */
_TS_Scheduler.prototype._warnNonFn = function (primitive, key) {
    var id = primitive + ":" + key;
    if (this._warnedNonFn[id]) return;
    this._warnedNonFn[id] = 1;
    _tsLog(primitive + ' "' + key + '" ignored: fn is not a function');
};

function _tsAutoDisableLimit(options) {
    var n = options ? _TS_safeNum(options.autoDisable, 0) : 0;
    return n > 0 ? n : 0;
}
...
function _tsFailTask(t, scopeId, name, e, label) {
    ...
    if (t.onError) {
        try { t.onError(e, name, scopeId); } catch (e2) {}
    }
...
/**
 * register(scopeId, name, interval, offset, fn[, options]) → handle {cancel}.
 * options: { internal: bool, isDirty: fn→bool, onError: fn(error, name) }.
 * A scope is heartbeat-driven: call heartbeat(scopeId) from your tile tick.
 * Returns false when validation fails.
 */
_TS_Scheduler.prototype.register = function (scopeId, name, interval, offset, fn, options) {
    if (scopeId === null || scopeId === undefined) return false;
    if (scopeId === "__auto" && !(options && options.__autoToken === _TS_AUTO_TOKEN)) return false;
    if (typeof name != "string" || !name) return false;
    if (this.reserved[name] && !(options && options.internal)) return false;
    ...
    var newOwner = (options && options.owner) || null;
    ...
    var task = {
        ...
        disabled: false,
        autoDisable: _tsAutoDisableLimit(options),
        lane: _tsLane(options && options.lane),
        owner: newOwner,
        isDirty: (options && typeof options.isDirty == "function") ? options.isDirty : null,
...
_TS_Scheduler.prototype._heartbeatBudgeted = function (scope, scopeId, extras) {
    ...
    var leftover = 0;
    // Small budget (< one task per lane): rotate the lane order so no lane
    // starves when the shares can't cover all lanes in a single tick.
    if (lanes.length > 1 && remaining < lanes.length) {
        var rrStart = this._rrLane % lanes.length;
        this._rrLane = (rrStart + 1) % lanes.length;
        lanes = lanes.slice(rrStart).concat(lanes.slice(0, rrStart));
    }
...
_TS_Scheduler.prototype.admin = function () {
    ...
    return {
        ...
        setBudget: function (options) { return self._setBudget(options); },
        getBudget: function () { return self._getBudget(); }
    };
};

/** Configure the per-tick budget (OFF by default). maxTasksPerTick default 64. */
_TS_Scheduler.prototype._setBudget = function (options) {
    options = options || {};
    if (options.enabled !== undefined) this._budget.enabled = !!options.enabled;
    if (options.maxTasksPerTick !== undefined) {
        var m = _TS_safeNum(options.maxTasksPerTick, this._budget.maxTasksPerTick);
        this._budget.maxTasksPerTick = m >= 1 ? m : _TS_BUDGET_DEFAULT;
    }
    if (options.laneWeights) {
        for (var li = 0; li < _TS_LANE_ORDER.length; li++) {
            var lane = _TS_LANE_ORDER[li];
            if (options.laneWeights[lane] !== undefined) {
                var w = Number(options.laneWeights[lane]);
 ...
 */
_TS_Scheduler.prototype.interval = function (name, ticks, fn, options) {
    ...
    var wrapper = function () { fn.apply(null, arguments); };
    wrapper._tsFn = fn;
    return this.register("__auto", name, ticks, options.offset || 0, wrapper,
        { internal: true, __autoToken: _TS_AUTO_TOKEN, owner: options.owner, lane: options.lane, autoDisable: options.autoDisable, isDirty: options.isDirty, onError: options.onError });
};
 ...
 */
_TS_Scheduler.prototype.ensureInterval = function (name, ticks, fn, options) {
    if (!this._driver) return false;
    if (typeof name != "string" || !name) return false;
    if (typeof fn != "function") return false;
    ticks = _TS_safeNum(ticks, 0);
 ...
 * calls reset the countdown. options: { ticks=10, maxTicks } — maxTicks forces a
 * run that many ticks after the FIRST call regardless of re-scheduling
 * (starvation guard). maxTicks < 1 disables the guard.
 */
_TS_Scheduler.prototype.debounce = function (key, fn, options) {
    if (typeof fn != "function") {
        this._warnNonFn("debounce", key);
        return { cancel: function () {} };
    }
    options = options || {};
    var ticks = _TS_safeNum(options.ticks, 10);
    if (ticks < 1) ticks = 1;
    var maxTicks = _TS_safeNum(options.maxTicks, 0);
 ...
 */
_TS_Scheduler.prototype.throttle = function (key, fn, options) {
    options = options || {};
    var ticks = _TS_safeNum(options.ticks, 10);
    if (ticks < 1) ticks = 1;
    var self = this;
 ...
 * options: { ticks=5, retries=3, reason, backoff="fixed"|"linear"|"exp", maxDelay, jitter } —
 * on exception the task is re-queued until `retries` TOTAL attempts are used up,
 * then dropped with a log. `ensureDefer(key, ...)` uses options.key internally.
 */
_TS_Scheduler.prototype.defer = function (fn, options) {
    options = options || {};
    if (typeof fn != "function") {
        this._warnNonFn("defer", (typeof options.key == "string" && options.key) ? options.key : (options.reason || "?"));
        return { cancel: function () {} };
    }
    var ticks = _TS_safeNum(options.ticks, 5);
    if (ticks < 1) ticks = 1;
    var retriesOpt = (options.attempts !== undefined) ? options.attempts : options.retries;
    var retries = _TS_safeNum(retriesOpt, 3);
    if (retries < 0) retries = 0;
    var backoff = options.backoff;
    if (backoff !== "linear" && backoff !== "exp") backoff = "fixed";
    var maxDelay = _TS_safeNum(options.maxDelay, 0);
    ...
    var entry = {
        ...
        key: (typeof options.key == "string" && options.key) ? options.key : null,
        cancelled: false
    };
    ...
    return {
        cancel: function () {
            ...
            if (entry.key && self._deferKeys[entry.key] === entry) delete self._deferKeys[entry.key];
        }
    };
};
 ...
 */
_TS_Scheduler.prototype.ensureDefer = function (key, fn, options) {
    if (typeof key != "string" || !key) return false;
    if (typeof fn != "function") return false;
    var self = this;
    var existing = this._deferKeys[key];
    if (existing && existing.cancelled) {
        delete this._deferKeys[key];
        existing = null;
    }
    if (existing) {
        if (existing.fn === fn) {
            return {
                cancel: function () {
                    ...
                    if (self._deferKeys[key] === existing) delete self._deferKeys[key];
                }
            };
        }
