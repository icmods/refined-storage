...
CraftTreeExecutor.createTask = function (fullCrafts, host, requestedItem, requestedCount, opts) {
    opts = opts || {};
    ...
    var task = {
        // opts.id lets the host own task identity; when
        // absent, the library generates a collision-resistant local id.
        id: opts.id || ("ct_" + Date.now() + "_" + (++_CP_taskIdCounter) + "_" + Math.floor(Math.random() * 1000000)),
        _host: host || {},
        ...
        startTime: (opts.startTime != null ? opts.startTime : undefined),
        cancelled: false,
