 ...
 * options: { inSlots?: string[], outSlots?: string[] } overrides the
 ...
 */
Adapters.container = function (storage, options) {
    options = options || {};
    var fixedIn = (options.inSlots && options.inSlots.length) ? options.inSlots.slice() : null;
    var fixedOut = (options.outSlots && options.outSlots.length) ? options.outSlots.slice() : null;
    var entry = {
        storage: 0,
        items_stored: 0,
        items: {},
 ...
 * is unchanged since the last successful syncAll is skipped (options.force
 * overrides). A throwing syncTo is reported (options.onError) and its token is
 * NOT marked, so the next call retries it. Returns
 * { leftover, synced, skipped, failed }.
 */
Adapters.syncAll = function (entries, options) {
    options = options || {};
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        if (!options.force && token !== null && entry._adpToken === token) {
            result.skipped++;
            continue;
        }
        var leftover = 0;
        try {
            ...
            if (typeof options.onError == "function") {
                try { options.onError(entry, e, i); } catch (e2) {}
            }
            continue;
        }
