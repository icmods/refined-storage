LIBRARY({
    ...
    shared: true,
    api: "CoreEngine",
    dependencies: ["TickScheduler:2"]
});

IMPORT("TickScheduler:2");
 ...
 *   options.onOpen(container, client)          — optional, AFTER the client is tracked
 *   options.onClose(uid, container, client)    — optional per-client cleanup callback
 * (onClose keeps uid as the first arg for backward compatibility.)
 */
MpCore.connectivity = function (tile, options) {
    options = options || {};
            ...
            container.addServerOpenListener({
                onOpen: function (cont, client) {
                    ...
                    if (typeof options.onOpen == "function") {
                        try { options.onOpen(cont || container, client, owner); } catch (e) {}
                    }
                }
            });
            tile.__mpOpenAttached = true;
            ...
            container.addServerCloseListener({
                onClose: function (cont, client) {
                    ...
                    if (typeof options.onClose == "function") {
                        try { options.onClose(uid, cont || container, client, owner); } catch (e) {}
                    }
                }
            });
            tile.__mpCloseAttached = true;
 ...
 */
MpCore.guardHandler = function (tile, handler, options) {
    options = options || {};
    var requireOpen = options.requireOpen === true;
    return function (eventData, connectedClient) {
        ...
        if (requireOpen && !tile.__mpClients[uid]) {
            if (typeof options.onReject == "function") {
                try { options.onReject(uid, tile); } catch (e) {}
            }
            return;
        }
 ...
 * options.requireNetworkEntity — skip tiles without a networkEntity (server-side
 *   detached tiles, e.g. created on an unloaded chunk).
 * options.allowRemoved — allow calling on tiles marked removed (default false).
 * Returns true when fn ran, false when skipped or when fn threw.
 */
MpCore.safeTileCall = function (tile, fn, options) {
    options = options || {};
    if (!tile || typeof fn != "function") return false;
    if (!tile.data || (!options.allowRemoved && tile.data.removed)) return false;
    if (options.requireNetworkEntity && !tile.networkEntity) return false;
    try { fn(tile); return true; }
    catch (e) {
        if (typeof Logger != "undefined" && Logger && Logger.Log) Logger.Log((options.label || "tileCall") + " failed: " + e, options.logTag || "MpCore");
        return false;
    }
};
