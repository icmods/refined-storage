 ...
 * dependencies + IMPORT("CoreKit:2")); the typeof-guard fallback below only
...
/*
 * Ephemeral per-entry index (never serialized): id -> positions plus a keyOf
 * cache per position. External writers must call invalidate() after mutating
 * a stacks entry directly.
 */

var _SC_meta = [];
/* Entry identity index: entries carry a small numeric handle (same pattern as the
 * host's _rsDirty marker), so metadata lookup stays O(1) instead of scanning. */
var _SC_metaById = Object.create(null);
var _SC_metaSeq = 0;

function _SC_metaFor(entry) {
    var id = entry._scMetaId;
    if (id != null) {
        var cached = _SC_metaById[id];
        if (cached && cached.entry === entry) return cached;
    }
    var meta = { entry: entry, byUid: null, keys: null, valid: false };
    _SC_metaSeq++;
    entry._scMetaId = _SC_metaSeq;
    _SC_metaById[entry._scMetaId] = meta;
    _SC_meta.push(meta);
    return meta;
}

function _SC_buildMeta(entry) {
    var stacks = (entry && entry.stacks) || [];
    var byUid = Object.create(null), keys = [];
    for (var i = 0; i < stacks.length; i++) {
        var stack = stacks[i];
        if (!stack || !stack.count) { keys[i] = undefined; continue; }
        var uid = _SC_uidOf(stack);
        keys[i] = uid;
        byUid[uid] = i;
    }
    return { byUid: byUid, keys: keys };
}

function _SC_rebuildMeta(entry) {
    var meta = _SC_metaFor(entry);
    var fresh = _SC_buildMeta(entry);
    meta.byUid = fresh.byUid;
    meta.keys = fresh.keys;
    meta.valid = true;
    return meta;
}

/* Positions after `pos` shifted by one: refresh the uid -> position index. */
function _SC_metaReindexFrom(meta, pos) {
    for (var i = pos; i < meta.keys.length; i++) {
        var uid = meta.keys[i];
        if (uid !== undefined) meta.byUid[uid] = i;
    }
}

StorageCore.invalidate = function (entry) {
    if (!entry) return false;
    _SC_metaFor(entry).valid = false;
    return true;
};

/* Drop the cached metadata of an entry that is no longer referenced (cache eviction). */
StorageCore.forgetEntry = function (entry) {
    if (!entry) return false;
    var id = entry._scMetaId;
    if (id != null) {
        var meta = _SC_metaById[id];
        if (meta) {
            delete _SC_metaById[id];
            for (var i = _SC_meta.length - 1; i >= 0; i--) {
                if (_SC_meta[i] === meta) { _SC_meta.splice(i, 1); break; }
            }
        }
        try { delete entry._scMetaId; } catch (e) {}
    }
    return true;
};

StorageCore.resetIndex = function () {
    _SC_meta = [];
    _SC_metaById = Object.create(null);
    _SC_metaSeq = 0;
};
 ...
 */
StorageCore.createView = function (sources, options) {
    return new _SCView(sources, options);
};
...
_SCView.prototype._collect = function () {
    ...
    for (var i = 0; i < this.sources.length; i++) {
        ...
        if (!entry) continue;
        if (entry.stacks && entry.stacks.length !== undefined) {
            var stacks = entry.stacks;
            for (var si = 0; si < stacks.length; si++) {
                var stack = stacks[si];
                if (!stack || !stack.count) continue;
                var stackKey = _SC_uidOf(stack);
                if (out[stackKey]) {
                    if (_SC_match(out[stackKey], stack, false)) out[stackKey].count += stack.count;
                    continue;
                }
                out[stackKey] = { id: stack.id, data: stack.data, extra: stack.extra, count: stack.count };
            }
            continue;
        }
 ...
 */
_SCView.prototype._addToEntry = function (index, uid, item, cap, trustedUid) {
    ...
    if (entry.stacks && entry.stacks.length !== undefined) {
        var meta = _SC_metaFor(entry);
        if (!meta.valid) _SC_rebuildMeta(entry);
        var pos = meta.byUid[uid];
        if (pos !== undefined && meta.keys[pos] === uid) {
            var stack = entry.stacks[pos];
        ...
        entry.stacks.push({ id: item.id, data: item.data, extra: item.extra || null, count: add });
        meta.keys.push(uid);
        meta.byUid[uid] = entry.stacks.length - 1;
        entry.items_stored = (entry.items_stored || 0) + add;
        return add;
    }
...
_SCView.prototype._opRecord = function (opId, remainder) {
    ...
    var cap = (this._options && this._options.opLogSize) || 128;
...
/**
 * Source indexes for insert routing. With options.selector, lower keys go first
...
_SCView.prototype._insertOrder = function (item) {
    ...
    var selector = this._options && this._options.selector;
 ...
 * options.onOverflow?(remainder, item) is notified when part of the insert does
 * not fit; exceptions inside the callback are isolated (swallowed).
 * Routing: options.selector (createView) orders sources; options.weight
 ...
 */
_SCView.prototype.insert = function (item, count, options) {
    count = _SC_safeNum(count);
    if (count <= 0 || !item || item.id == 0) return count;
    if (options && options.opId != null) {
        var seen = this._opSeen(options.opId);
        if (seen !== undefined) return seen;
    }
    var trustedUid = !!(options && options.trustedUid);
    var uid = (options && options.uid != null) ? options.uid : _SC_uidOf(item);
    var original = count;
    var remaining = count;
    var order = this._insertOrder(item);
    var weightFn = this._options && this._options.weight;
    if (typeof weightFn == "function") {
        ...
        if (totalW > 0) {
            for (var si = 0; si < shares.length && remaining > 0; si++) {
                ...
                if (share <= 0) continue;
                remaining -= this._addToEntry(shares[si].index, uid, item, Math.min(share, remaining), trustedUid);
            }
        }
    }
    for (var oi = 0; oi < order.length && remaining > 0; oi++) {
        remaining -= this._addToEntry(order[oi], uid, item, remaining, trustedUid);
    }
    if (options && options.opId != null) this._opRecord(options.opId, remaining);
    _SC_emitChange(this, uid, original - remaining, item, "insert");
    if (remaining > 0 && options && typeof options.onOverflow == "function") {
        try { options.onOverflow(remaining, item); } catch (e) {}
    }
    return remaining;
};
 ...
 */
_SCView.prototype.extract = function (uid, count, options) {
    count = _SC_safeNum(count);
    if (count <= 0) return 0;
    if (options && options.opId != null) {
        var seenEx = this._opSeen(options.opId);
    ...
    for (var i = 0; i < this.sources.length && remaining > 0; i++) {
        ...
        if (entry.stacks && entry.stacks.length !== undefined) {
            ...
            var pos = meta.byUid[uid];
            if (pos !== undefined && meta.keys[pos] === uid) {
                var candidateStack = entry.stacks[pos];
                if (candidateStack && candidateStack.count) {
                    var candidateStored = Number(candidateStack.count);
                    if (isFinite(candidateStored) && candidateStored > 0) {
                        var candidateTake = Math.min(remaining, candidateStored);
                        candidateStack.count = candidateStored - candidateTake;
                        entry.items_stored = Math.max(0, (entry.items_stored || 0) - candidateTake);
                        remaining -= candidateTake;
                        if (candidateStack.count <= 0) {
                            entry.stacks.splice(pos, 1);
                            meta.keys.splice(pos, 1);
                            delete meta.byUid[uid];
                            _SC_metaReindexFrom(meta, pos);
                        }
                    }
                }
            } else {
                for (var li = 0; li < entry.stacks.length && remaining > 0; li++) {
                    if (meta.keys[li] !== uid) continue;
                    var scanStack = entry.stacks[li];
                    if (!scanStack || !scanStack.count) continue;
                    var scanStored = Number(scanStack.count);
                    if (!isFinite(scanStored) || scanStored <= 0) continue;
                    var scanTake = Math.min(remaining, scanStored);
                    scanStack.count = scanStored - scanTake;
                    entry.items_stored = Math.max(0, (entry.items_stored || 0) - scanTake);
                    remaining -= scanTake;
                    if (scanStack.count <= 0) {
                        entry.stacks.splice(li, 1);
                        meta.keys.splice(li, 1);
                        delete meta.byUid[uid];
                        _SC_metaReindexFrom(meta, li);
                        break;
                    }
                }
            }
...
_SCTransaction.prototype._buildShadow = function () {
    ...
    for (var i = 0; i < this.view.sources.length; i++) {
        ...
        if (!src) { shadow.push(null); continue; }
        if (src.stacks && src.stacks.length !== undefined) {
            var stacksCopy = [];
            for (var si = 0; si < src.stacks.length; si++) {
                var stack = src.stacks[si];
                if (!stack) continue;
                stacksCopy.push({ id: stack.id, data: stack.data, extra: stack.extra, count: stack.count });
            }
            shadow.push({ storage: src.storage || 0, items_stored: src.items_stored || 0, stacks: stacksCopy });
            continue;
        }
        if (!src.items) { shadow.push(null); continue; }
...
StorageCore.debug = {
    ...
    validate: function (view) {
        ...
        for (var i = 0; i < sources.length; i++) {
            ...
            if (entry.stacks && entry.stacks.length !== undefined) {
                var actualStacks = 0;
                for (var si = 0; si < entry.stacks.length; si++) {
                    var stack = entry.stacks[si];
                    if (!stack) {
                        mismatches.push({ index: i, reason: "null-item", expected: 0, actual: 0 });
                        continue;
                    }
                    var stackCount = Number(stack.count);
                    if (!isFinite(stackCount) || stackCount < 0) {
                        mismatches.push({ index: i, reason: "invalid-count", expected: 0, actual: stack.count });
                        continue;
                    }
                    actualStacks += stackCount;
                }
                var expectedStacks = Number(entry.items_stored) || 0;
                if (actualStacks !== expectedStacks) {
                    mismatches.push({ index: i, reason: "bookkeeping-drift", expected: expectedStacks, actual: actualStacks });
                }
                var storageStacks = Number(entry.storage);
                if (isFinite(storageStacks) && expectedStacks > storageStacks) {
                    mismatches.push({ index: i, reason: "over-capacity", expected: storageStacks, actual: expectedStacks });
                }
                continue;
            }
            if (!entry.items) continue;
    ...
    /**
     * Check the cached index against the stacks; a stale index is rebuilt
     * in place unless `repair` is false.
     */
    validateIndex: function (entry, repair) {
        if (!entry || !(entry.stacks && entry.stacks.length !== undefined)) return { ok: true, mismatches: [] };
        var meta = _SC_metaFor(entry);
        var fresh = _SC_buildMeta(entry);
        if (!meta.valid) {
            meta.byUid = fresh.byUid;
            meta.keys = fresh.keys;
            meta.valid = true;
            return { ok: true, rebuilt: true, mismatches: [] };
        }
        var mismatches = [];
        var max = Math.max(meta.keys.length, fresh.keys.length);
        for (var i = 0; i < max; i++) {
            if (meta.keys[i] !== fresh.keys[i]) {
                mismatches.push({ reason: "keys", index: i, expected: fresh.keys[i], actual: meta.keys[i] });
            }
        }
        var idsA = [], idsB = [];
        for (var a in meta.byUid) idsA.push(a);
        for (var b in fresh.byUid) idsB.push(b);
        if (idsA.sort().join(",") !== idsB.sort().join(",")) mismatches.push({ reason: "ids" });
        if (mismatches.length && repair !== false) {
            meta.byUid = fresh.byUid;
            meta.keys = fresh.keys;
        }
        return { ok: mismatches.length === 0, mismatches: mismatches };
