 ...
 * options.onFlush(lines) is called every 10 wraps of the ring — the host can
 * persist lines through FileTools.WriteText for crash-safe diagnostics.
 */
DebugCore.ring = function (capacity, options) {
    options = options || {};
    ...
    return {
        push: function (line) {
            ...
            if (buf.length > capacity) {
                buf.shift();
                wrapped++;
                if (wrapped % 10 === 0 && typeof options.onFlush == "function") {
                    try { options.onFlush(buf.slice()); } catch (e) {}
                }
            }
        },
        getLast: function (n) {
 ...
 * options: { tag, title, dialog: false }
 */
DebugCore.report = function (error, options) {
    options = options || {};
    var message = String(error && (error.stack || error.message || error) || error || "");
    var text = "[" + (options.tag || "ERROR") + "] " + message;
    try {
        if (typeof Logger != "undefined" && Logger && Logger.Log) Logger.Log(text, "DebugCore");
    } catch (e) {}
    if (_DBG_ring) _DBG_ring.push("ERROR " + text);
    if (options.dialog !== false) DebugCore.showReport(message, options.title);
    return message;
};
