...
/**
 * on(event, fn[, scope][, options]) — options: { owner, ephemeral }.
 * Returns a stable numeric subscription id (no longer the list index).
 */
_EBBus.prototype.on = function (event, fn, scope, options) {
    ...
    var entry = {
        ...
        owner: (options && options.owner) || null,
...
_EBBus.prototype.once = function (event, fn, scope, options) {
    var self = this;
    ...
    this.on(event, wrapper, scope, options);
    return wrapper;
};
...
EventBus.ns = function (name) {
    ...
    var handle = {
        name: name,
        on: function (event, fn, options) { return bus.on(key(event), fn, null, { owner: name, ephemeral: options && options.ephemeral }); },
        once: function (event, fn, options) { return bus.once(key(event), fn, null, { owner: name, ephemeral: options && options.ephemeral }); },
